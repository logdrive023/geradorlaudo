...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-4 rounded-lg bg-slate-800/80 backdrop-blur-sm">
      <h3 className="text-xl font-bold mb-6 text-white">Envie uma mensagem</h3>

      {status.type && (
        <Alert
          variant={status.type === "error" ? "destructive" : "default"}
          className={`p-4 ${status.type === "error" ? "bg-red-900/50 border-red-500/50" : "bg-green-900/50 border-green-500/50"}`}
        >
          <AlertDescription className="text-white font-medium">{status.message}</AlertDescription>
        </Alert>
      )}
